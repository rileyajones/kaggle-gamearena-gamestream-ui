...
import './style.css';
import { getActionString, hasAction } from "../../utils/step";
...
export const Moves = () => {
  ...
  const stepsByModelIndex = models.map((model) => {
    return steps.flat().filter((step) => step.modelId === model.id && hasAction(step));
  });
    ...
    <div className="moves-table-container" ref={movesContainer}>
      <table className="moves-table">
        ...
        <tbody>
          {rows.map((steps) =>
            <tr>
              {steps.map((step) => <td>{getActionString(step) ?? ''}</td>)}
            </tr>
          )}
        </tbody>
      </table>
